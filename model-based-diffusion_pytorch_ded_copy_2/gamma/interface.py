...
class FeaModel():
    ...
    def run_onestep(self,given_laser_power=None):
        ''' Run the simulation onestep. '''
        ...
        self.active_nodes_previous = self.domain.active_nodes.astype('i1')
        if self.domain.current_sim_time < self.domain.end_sim_time - 1e-8 and self.heat_solver.current_step < self.max_itr :
            self.step(given_laser_power)
    ...
    def step(self, laser_power=None):
        ...
        else: # if laser power is given at this step
        ...
        self.free_memory_counter += 1

        if self.free_memory_counter >= 200:
            mempool = cp.get_default_memory_pool()
            mempool.free_all_blocks()
            self.free_memory_counter = 0
        ...
        #print(f"======= solve step {self.heat_solver.current_step} =========")
