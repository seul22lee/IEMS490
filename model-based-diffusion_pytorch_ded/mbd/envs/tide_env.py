...
class TiDEEngineeringEnv:
    def __init__(self, weight_path: str, device="cuda"):
        ...
        self.action_size = self.model.future_cov_dim
    ...
    def predict_one_shot(self, state_history, u_sequence):
        Nsample = u_sequence.shape[0]
        past_cov = state_history.repeat(Nsample, 1, 1).to(self.device)
        
        with torch.no_grad():
            # u_sequence: [Nsample, H, act]
            outputs = self.model([past_cov, u_sequence, None])
