...
class TiDEEngineeringEnv:
    def __init__(self, weight_path: str, device="cuda"):
        ...
        try:
            ...
            # case 1 : 기존 format (model 저장된 경우)
            if 'model' in params:
                self.model = params['model']

            # case 2 : checkpoint format (state_dict 저장된 경우)
            elif 'models_state_dict' in params:

                state_dict = params["models_state_dict"][0]
                P = params["P"]
                ...
                raise RuntimeError("Unknown TiDE weight format")
        ...
        self.observation_size = self.model.output_dim
        self.future_cov_dim_total = self.model.future_cov_dim   # should be 4
        self.action_size = 1                                    # only laser power is optimized
