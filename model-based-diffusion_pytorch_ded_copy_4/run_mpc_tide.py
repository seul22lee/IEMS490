...
def plot_fig(MPC_GAMMA, N_step, save_path=None):
    ...
    plt.axhline(y=0.225, color='k', linestyle='--'); plt.axhline(y=0.075, color='k', linestyle='--')
    plt.ylabel("Depth (mm)")
    plt.subplot(3, 1, 3)
...
def run_one_step_diffusion(GAMMA_obj, env, mbd_args, P, window):
    ...
    state_init_s = torch.cat((mp_temp_past_s, fix_cov_past_s, laser_past_s), dim=2)
    ...
    u_prev = normalize_x(GAMMA_obj.u_past[-1].view(1, 1), dim_id=[3]).to(device)
...
if __name__ == "__main__":
    ...
    def build_exp_name(args):
        ...
        return (
            f"Ns{args.Nsample}_"
            f"H{args.Hsample}_"
            f"Nd{args.Ndiffuse}_"
            f"T{args.temp_sample}_"
            # f"b0{args.beta0}_"
            # f"bT{args.betaT}_"
            f"tr{args.w_tracking}_"
            f"sm{args.w_smooth}_"
            f"con{args.w_constraint}_"
            f"u0{args.w_u0}_"
    ...
    fix_covariates_np = np.concatenate((loc_Z, dist_X, dist_Y), axis=1)
    ...
    for exp_cfg in EXPERIMENTS:
        ...
        try:
            ...
            for k,v in exp_cfg.items():
                setattr(mbd_args, k, v)

            exp_name = build_exp_name(mbd_args)

            print("\n==============================")
            print("Running experiment:", exp_name)
            print("==============================")

            save_dir = f"./results/plots/laser_{laser_num}/{exp_name}"
            os.makedirs(save_dir, exist_ok=True)

            with open(os.path.join(save_dir,"args.json"),"w") as f:
                json.dump(vars(mbd_args),f,indent=4)
            ...
            fix_covariates = torch.tensor(fix_covariates_np,dtype=torch.float32).to(device)
            laser_power_ref = torch.tensor(laser_power_ref_np,dtype=torch.float32).to(device)
            ...
            laser_plot_dir = os.path.join(save_dir,"trajectory")
            os.makedirs(laser_plot_dir,exist_ok=True)

            for i in tqdm(range(N_step-P),desc=exp_name):
                ...
                if current_step % 100 == 0:
                    ...
                    plot_fig(
                        GAMMA_class,
                        GAMMA_class.MPC_counter,
                        save_path=mid_plot_path
                    )

            final_plot_path = os.path.join(save_dir,"final_plot.png")
            # ==========================================
            # CSV 저장
            # ==========================================

            temp_series = GAMMA_class.x_past_save[:,0].detach().cpu().numpy()
            depth_series = GAMMA_class.x_past_save[:,1].detach().cpu().numpy()
            laser_series = GAMMA_class.u_past_save[:,0].detach().cpu().numpy()
            ref_series = GAMMA_class.ref[:len(temp_series)].detach().cpu().numpy()
            ...
            print("trajectory csv saved:", csv_path)
