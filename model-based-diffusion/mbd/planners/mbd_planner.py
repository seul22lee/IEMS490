...
def run_diffusion(args: Args):
    ...
    YN = jnp.zeros([args.Hsample, Nu]) # seul // warm start using last action

    @jax.jit # seul // gpu parallelization
    def reverse_once(carry, unused):
        ...
        Y0s = jnp.clip(Y0s, -1.0, 1.0) # seul // sampled control sequences ; n

        # esitimate mu_0tm1
        rewss, qs = jax.vmap(rollout_us, in_axes=(None, 0))(state_init, Y0s) # seul // dynamics rollout , rewss: reward trajectory, qs: state trajectory
        rews = rewss.mean(axis=-1) # seul // reward mean (over trajectory)
        rew_std = rews.std() # seul // reward std (over trajectory)
        rew_std = jnp.where(rew_std < 1e-4, 1.0, rew_std)
        rew_mean = rews.mean()
        logp0 = (rews - rew_mean) / rew_std / args.temp_sample # seul // args.temp_sample is the temperature for sampling, which controls the exploration-exploitation trade-off. A higher temperature encourages more exploration, while a lower temperature encourages more exploitation of high-reward actions.
        ...
        score = 1 / (1.0 - alphas_bar[i]) * (-Yi + jnp.sqrt(alphas_bar[i]) * Ybar) # seul // score function ; How to move Yi to Ybar direction
    ...
    def reverse(YN, rng):
        Yi = YN
        Ybars = []
        # seul // diffusion process : iteratively refine the control sequence Yi by moving it towards higher-reward regions in the action space, guided by the score function and the sampled control sequences Y0s.
